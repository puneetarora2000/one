...
/**
 * Default implementation of the {@link IAnnotationStudy} interface.
 * Abstract base class for coding and unitizing annotation studies. 
...
public abstract class AnnotationStudy implements IAnnotationStudy {
	...
	/** Add a rater with the given name. Returns the index of the newly added
	 *  rater which is required to identify the rater in 
	 *  {@link IAnnotationUnit}. The first rater receives index 0. 
	 *  Note that adding names for the raters is optional. */
	...
	/** Adds the given category to the set of possible annotation labels. This 
	 *  method is only required of a category has not been used by any rater of 
	 *  within the annotation study.
	...
	/*public Object[] getCategoryArray() {
		...
		for (Object category : getCategories()) {
			if (result == null)
				result = new Object[categories.size()];
			result[idx++] = category;
		}
		return result;
	}*/
